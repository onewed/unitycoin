...
module.exports = (deployer, network, accounts) => {
    let totalSupply, minimumGoal, minimumContribution, maximumContribution, walletAddress, deployAddress, start, hours, isPresale, discounts;
    if (network === 'development') {
        ...
        // deployAddress = ''; // TODO: set this address!
        // walletAddress = ''; // TODO: set this address!
        totalSupply = new BigNumber(4 * 10**9 * 10**18); // 4 billion tokens, but converted to 18 decimal places, like ether to wei
        minimumGoal = new BigNumber(35000 * 10**18); // 35000 ETH in wei ($10.5M @ $302.50/ETH)
        minimumContribution = new BigNumber(10**18); // 1 ETH in wei;
        maximumContribution = new BigNumber(200 * 10**18); // 200 ETH in wei;
        start = Math.ceil((new Date()).getTime() / 1000);
        isPresale = true;
        hours = 120; // 5 days in hours
        discounts = [
